...
begin
  ...
  bd := StrToBigDecimal('1.2'); test(BigDecimalToStr(bd), '1.2');  test(BigDecimalToStr(bd, bdfExponent), '1.2E0');
  bd := StrToBigDecimal('1.0'); test(BigDecimalToStr(bd), '1'); test(BigDecimalToStr(bd, bdfExponent), '1.0E0');
  bd := StrToBigDecimal('1.000'); test(BigDecimalToStr(bd), '1'); test(BigDecimalToStr(bd, bdfExponent), '1.0E0');
  bd := StrToBigDecimal('1.'); test(BigDecimalToStr(bd), '1'); test(BigDecimalToStr(bd, bdfExponent), '1.0E0');
  bd := StrToBigDecimal('1.234567890234567890234567890'); test(BigDecimalToStr(bd), '1.23456789023456789023456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789023456789023456789E0');
  bd := StrToBigDecimal('1.234567890'); test(BigDecimalToStr(bd), '1.23456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789E0');
  bd := StrToBigDecimal('1.23456789000000000000000000'); test(BigDecimalToStr(bd), '1.23456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789E0');
  bd := StrToBigDecimal('121121232343.234567890234567890234567890'); test(BigDecimalToStr(bd), '121121232343.23456789023456789023456789');
  test(BigDecimalToStr(bd, bdfExponent), '1.2112123234323456789023456789023456789E11');
  bd := StrToBigDecimal('0.1'); test(BigDecimalToStr(bd), '0.1');
  ...
  bd := StrToBigDecimal('1234567890E-10'); test(BigDecimalToStr(bd), '0.123456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789E-1');
  bd := StrToBigDecimal('123456789.0E-10'); test(BigDecimalToStr(bd), '0.0123456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789E-2');
  bd := StrToBigDecimal('1234567.890E-10'); test(BigDecimalToStr(bd), '0.000123456789'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789E-4');
  bd := StrToBigDecimal('1234567.89000000000000000000000007E-10'); test(BigDecimalToStr(bd), '0.000123456789000000000000000000000007'); test(BigDecimalToStr(bd, bdfExponent), '1.23456789000000000000000000000007E-4');
  bd := StrToBigDecimal('-10.6832'); test(BigDecimalToStr(bd), '-10.6832'); test(BigDecimalToStr(bd, bdfExponent), '-1.06832E1');
  bd := StrToBigDecimal('-10.683215'); test(BigDecimalToStr(bd), '-10.683215');test(BigDecimalToStr(bd, bdfExponent), '-1.0683215E1');
  bd := StrToBigDecimal('-10.6832154'); test(BigDecimalToStr(bd), '-10.6832154'); test(BigDecimalToStr(bd, bdfExponent), '-1.06832154E1');
  bd := StrToBigDecimal('.123'); test(BigDecimalToStr(bd), '0.123'); test(BigDecimalToStr(bd, bdfExponent), '1.23E-1');
  bd := StrToBigDecimal('-.1'); test(BigDecimalToStr(bd), '-0.1'); test(BigDecimalToStr(bd, bdfExponent), '-1.0E-1');
...
//  for i := 0 to 100 do begin
  ...
  test(BigDecimalToStr(FloatToBigDecimal(single(3.14159), true)), '3.141590118408203125');
  test(BigDecimalToStr(FloatToBigDecimal(single(0.81), true)), '0.810000002384185791015625');
  test(BigDecimalToStr(FloatToBigDecimal(single(144115188075855877), true)), '144115188075855872');
  test(BigDecimalToStr(FloatToBigDecimal(single(1 / 3.0), true)), '0.3333333432674407958984375');
  test(BigDecimalToStr(FloatToBigDecimal(single(1E-20), true)), '0.000000000000000000009999999682655225388967887463487205224055287544615566730499267578125');
  test(BigDecimalToStr(FloatToBigDecimal(single(92233720368547758), true)), '92233718306963456');
  test(BigDecimalToStr(FloatToBigDecimal(single(3.4028235E38), true)), '340282346638528859811704183484516925440');



  test(BigDecimalToStr(FloatToBigDecimal(single(3.14159), false)), '3.14159');
  test(BigDecimalToStr(FloatToBigDecimal(single(0.81), false)), '0.81');
  test(BigDecimalToStr(FloatToBigDecimal(single(144115188075855877), true)), '144115188075855872');
  test(BigDecimalToStr(FloatToBigDecimal(single(1 / 3.0), false)), '0.33333333');
  test(BigDecimalToStr(FloatToBigDecimal(single(1E-20), false)), '0.00000000000000000001');
  test(BigDecimalToStr(FloatToBigDecimal(single(92233720368547758), false)), '92233720000000000');
  test(BigDecimalToStr(FloatToBigDecimal(single(3.4028235E38), false)), '340282340000000000000000000000000000000');


  test(BigDecimalToStr(FloatToBigDecimal(double(3.14159), true)), '3.14158999999999988261834005243144929409027099609375');
  test(BigDecimalToStr(FloatToBigDecimal(double(0.81), true)), '0.810000000000000053290705182007513940334320068359375');
  test(BigDecimalToStr(FloatToBigDecimal(double(144115188075855877), true)), '144115188075855872');
  test(BigDecimalToStr(FloatToBigDecimal(double(1.2648080533535911530920161097426467149E-321), true)), '0.');
  test(BigDecimalToStr(FloatToBigDecimal(double(6.9533558078350043221569772416637627063E-310), true)), '0.');

  test(BigDecimalToStr(FloatToBigDecimal(double(3.14159))), '3.14159');
  test(BigDecimalToStr(FloatToBigDecimal(double(0.81))), '0.81');
  test(BigDecimalToStr(FloatToBigDecimal(double(144115188075855877))), '144115188075855860');
  ...
  test(BigDecimalToStr(FloatToBigDecimal(double(-1.7976931348623158e+30))), '-1797693134862315700000000000000');
  test(BigDecimalToStr(FloatToBigDecimal(double(1.7976931348623158e+30))), '1797693134862315700000000000000');
  ...
  test(BigDecimalToStr(FloatToBigDecimal(double(1.2648080533535911530920161097426467149E-321), false)), '0.');
  test(BigDecimalToStr(FloatToBigDecimal(double(6.9533558078350043221569772416637627063E-310), false)), '0.');



  test(BigDecimalToStr(FloatToBigDecimal(extended(3.14159), true)), '3.14158999999999999992901511536302905369666405022144317626953125');
  test(BigDecimalToStr(FloatToBigDecimal(extended(1e-20), true)), '0.0000000000000000000100000000000000000003416395822511968001989078435098753026666395786361922481955133434894378297030925750732421875');
  test(BigDecimalToStr(FloatToBigDecimal(extended(1 / extended(3)), true)), '0.33333333333333333334236835143737920361672877334058284759521484375');
  {                                                                                    x
  ...
  test(BigDecimalToStr(FloatToBigDecimal(extended(e), true)), '');
  test(BigDecimalToStr(FloatToBigDecimal(extended(92233720368547758), true)), '92233720368547758');
