...
[[functiont, itemStar]]}
  ...
  fn3_1.registerInterpretedFunction('xml-to-json', '($input as node()?, $options as map(*)) as xs:string?', ' $input ! (let $json := x:joewiz-xml-to-json-recurse($input) let $serialization-parameters := map { "method": "json", "indent": $options?indent } return serialize($json, $serialization-parameters))');
